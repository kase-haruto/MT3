...
bool IsCollision(const Sphere* s, const Plane* plane){
	...
	Vector3 pos;
	pos.x= planeNormal.x * planeDistance;
	pos.y= planeNormal.y * planeDistance;
	pos.z= planeNormal.z * planeDistance;
	float distance;
...
bool IsCollision(const Segment& segment, const Triangle* triangle){
	...
	collisionPoint.x = segment.origin.x + segment.diff.x * t;
	collisionPoint.y = segment.origin.y + segment.diff.y * t;
	collisionPoint.z = segment.origin.z + segment.diff.z * t;
...
bool isCollision(const Segment& segment, const AABB* aabb){
	...
	Vector3 t1;
	t1.x = (aabb->GetMin().x - segment.origin.x) * invDir.x;
	t1.y = (aabb->GetMin().y - segment.origin.y) * invDir.y;
	t1.z = (aabb->GetMin().z - segment.origin.z) * invDir.z;
	Vector3 t2;
	t2.x = (aabb->GetMax().x - segment.origin.x) * invDir.x;
	t2.y = (aabb->GetMax().y - segment.origin.y) * invDir.y;
	t2.z = (aabb->GetMax().z - segment.origin.z) * invDir.z;
